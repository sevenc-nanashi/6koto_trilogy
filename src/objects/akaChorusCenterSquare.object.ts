...
export default defineObject({
  id: "aka-chorus-center-square",
  label: "Aka Chorus Center Square",
  parameters: {
    circleProgress: {
      type: "number",
  ...
  draw(ctx, p, params) {
    ...
    const lastKick = akaDrumsMidiTrack.notes.findLast(
      (note) =>
    ...
    const kickElapsed = lastKick
      ? frame.measure -
        akaMidi.toneJsMidi.header.ticksToMeasures(lastKick.ticks)
      : Infinity;
    const lastSnare = akaDrumsMidiTrack.notes.findLast(
      (note) =>
    ...
    const snareElapsed = lastSnare
      ? frame.measure -
        akaMidi.toneJsMidi.header.ticksToMeasures(lastSnare.ticks)
      : Infinity;
    const snareRadius =
      baseSize / 2 -
      easeOutQuint(clip(snareElapsed * 4)) * snareExpand * params.activate;

    const lastCymbal: DrumPattern | undefined = akaDrumsMidiPatterns.findLast(
      (pattern) =>
    ...
    if (lastCymbal) {
      const elapsed =
        frame.measure -
        akaMidi.toneJsMidi.header.ticksToMeasures(lastCymbal.ticks);
